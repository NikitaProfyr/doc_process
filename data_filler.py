...
from services import insert_data_table, get_fields_documents_tbl
...
async def run_data_filler():
    """Заполняет данными таблицы documents и data в базе данных"""
    data = make_data()
    # данные для базы:
    ...
    for item in data_tbl:
        await insert_data_table(table_name="data", connect=connect, fields=item)

    for item in documents_tbl:
        fields = get_fields_documents_tbl(item=item)
        await insert_data_table(table_name="documents", connect=connect, fields=fields)

    await connect.close()
    print("Данные успешно записаны!")


asyncio.run(run_data_filler())
